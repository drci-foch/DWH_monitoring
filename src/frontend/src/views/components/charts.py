...
class ChartDisplay:
    ...
    def create_document_distribution_chart(
        doc_counts: List[Dict[str, Any]], title: str
    ) -> None:
        """
    ...
    def create_time_series_chart(
        ...
        fig = px.line(
            df, x=time_column, y="count", color="document_origin_code", title=title
        )
    ...
    def create_archive_chart(archive_data: Dict[str, Any]) -> None:
        ...
        """
        df = pd.DataFrame(
            archive_data["documents_to_suppress"], columns=["Origine", "Nombre"]
        )
        ...
        fig.add_trace(
            go.Bar(
                ...
                y=df["Nombre"],
                text=df.apply(
                    lambda x: f"{x['Nombre']:,}<br>{x['Pourcentage']:.1f}%", axis=1
                ),
